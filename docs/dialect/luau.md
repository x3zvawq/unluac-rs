...
`packages/unluac-test-support/src/case_manifest.rs` 需要新增 `LuaCaseDialect::Luau`，
...
`packages/unluac-test-support/src/lib.rs` 需要把“选择 runtime/compiler 可执行文件名”升级成
...
- `packages/unluac-test-support/src/lib.rs`
- `packages/unluac-test-support/src/case_manifest.rs`
...
如果 `packages/unluac-test-support/src/lib.rs` 继续假设所有 dialect 都是 `lua` / `luac`，
...
- 对外只保留单一 `luau` dialect，不拆 `luau3` / `luau4` / `luau5` / `luau6` / `luau7`
...
Luau 的 bytecode 版本演进方式，与 PUC-Lua 5.1-5.5 这类“按语言版本拆目录”的路线
并不一样。

官方 bytecode header 本身就显式携带:

- `bytecode_version`
- `type_version`

而官方 loader 也是在单一解析入口里按版本做少量分支，而不是为每个版本维护一整套
独立 loader。

因此，本仓库后续如果要扩展 Luau 多版本支持，推荐保持以下策略:

- 对外公开接口继续只有一个 `luau` dialect
- 在 parser / transformer 内部按 `bytecode_version + type_version` 做 capability 分支
- 不把 `luau6` / `luau7` 暴露成新的公开 dialect 名字

原因:

- Luau 3-7 的差异主要是增量能力，而不是完全不同的字节码 family
- 真实需要建模的分支点，主要集中在 header / proto 附加字段 / 常量 tag / opcode 可用集
- 如果过早拆出多个公开 dialect，测试矩阵、CLI、文档和代码路径都会膨胀，但收益很小

目前对版本差异的理解可以粗分为:

- `v3`: baseline of currently supported public versions，包含 `FORGPREP` / `JUMPXEQK*` 等更新
- `v4`: 增加 `Proto::flags`、`typeinfo`、`IDIV/IDIVK`
- `v5`: 增加 `SUBRK/DIVRK` 与 vector constants
- `v6`: 增加 `FASTCALL3`
- `v7`: 增加 `LBC_CONSTANT_TABLE_WITH_CONSTANTS`

其中，真正影响 parser 布局的关键分支主要是:

- `bytecode_version >= 4`: header 多一个 `type_version`，proto 多 `flags + typeinfo`
- `type_version == 3`: 需要额外解析 userdata type mapping
- `bytecode_version >= 7`: 常量池里可能出现 `TABLE_WITH_CONSTANTS`

这意味着:

- Luau 更适合走“单一 parser + 内部版本 capability”路线
- 不适合照搬 PUC-Lua 那种 `lua51` / `lua52` / `lua53` 的公开 dialect 拆法

### 当前承诺边界

当前仓库真正完成验证的范围是:

- vendored `luau-compile` 产出的 `bytecode_version=7`
- `type_version=3`

这也是当前 `tests/lua_cases/luau` 与 Luau 相关 pipeline 验证实际覆盖到的版本。

对于 Luau 3-6:

- parser 结构上已经朝“可版本分支”的方向设计
- 但目前还没有形成正式兼容矩阵与 fixture 覆盖
- 因此不应在文档里承诺“已支持所有 Luau 3-7 bytecode”

对于 Luau 1-2:

- 官方说明里提到 version 2 与 version 3 之间存在历史兼容歧义
- 这类旧 blob 不适合当成第一优先级目标
- 如果未来确实要支持，应作为 legacy compatibility work 单独评估
...
- `DecompileDialect::Luau` 已接入
- tests / toolchain 已按 dialect 选择 `luau` / `luau-compile`
- Luau parser / transformer / AST / generate 主线已打通
- `tests/lua_cases/luau` 可以走完整条 decompile pipeline
- 生成结果以“Luau 可执行、可重新编译”为目标，不恢复类型
- `tests/lua_cases/common` 与 `tests/lua_cases/luau` 在当前 unit matrix 下已通过

换句话说，第一阶段“支持 Luau bytecode 反编译到可执行源码”的目标已经基本达成。

### TODO

后续仍值得继续推进的事项:

- 为 Luau 3-7 建立更明确的内部 capability 表，而不是只在局部代码里按版本分支
- 补充不同 `bytecode_version` / `type_version` 的 fixture 与回归测试
- 明确 parser 对“未验证 Luau 版本”的错误信息与支持边界
- 继续优化生成结果的可读性，但不以恢复 typed Luau 为目标
- 如未来确有需求，再单独评估 Luau 1-2 的 legacy compatibility

当前不着急做的事情:

- typed Luau 恢复
- Luau-specific syntax sugar 恢复
- 将 Luau 再拆成多个公开 dialect 名称
