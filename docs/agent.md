...
- 当前仍待处理的审计问题与复现入口见 [audit.md](./audit.md)，接手新主题前先核对优先级和已完成基线，避免重复修复或回退已经确认的生成契约。
...
- 不得仅因改动范围大而回避高价值架构修复。若根因涉及层次职责、核心数据模型或跨 pass 所有权，应先明确边界、不变量、迁移范围和验证方案，再直接替换旧结构；不要用兼容层、双轨事实或后层 fallback 保留已确认错误的架构。只有收益不足、边界无法证明或验证条件不成立时才缩小方案，并应明确说明取舍。
...
- HIR / AST pass 必须遵守 [Pass guard 合同](./design.md#pass-guard-合同)，分类、标记与审计完成标准以该文档为唯一规范。
