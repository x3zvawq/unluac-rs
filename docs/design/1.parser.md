...
12. **Lua 5.5 绝对行号项不是 varint**：`abslineinfo` 数量使用 Lua 5.5 varint 编码；非空表先对齐到 header 声明的 `int` 宽度，随后每项按 chunk 字节序读取两个定宽 `int`（`pc`、`line`）。不能把项本身继续当 varint，否则会错位到后续 local/upvalue name 字符串。
