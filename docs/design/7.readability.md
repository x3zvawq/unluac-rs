...
| `binding_tree.rs` | binding 树结构分析；`captured_bindings` 这类闭包创建点的父级槽位依赖也在这里统一查询 |
...
5. **`statement_merge` 尊重 goto/label 边界和前缀读取**：不能把 `local` 下沉到 forward goto 之后，否则生成"跳进 local 作用域"的非法 Lua；也不能把 hoisted temp 下沉到已经读取过该 binding 的语句之后，否则 fallback/goto 回边会读到尚未进入作用域的新 local。
...
8. **链式 method-call 只能吃掉死 local**：`local x = a:b(); x:c()` 折成 `a:b():c()` 前，必须确认 `x` 在后续语句没有继续作为 receiver、字段基底或闭包捕获被读取。
