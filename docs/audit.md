...
8. **repeat carried 与 live-out identity 尚未合一**：`regress_201` 全 7 方言把源码 `maximum/minimum` 拆成 per-iteration next、loop-carried 与 post-loop result 多套 temp/local。可合并的边界是无 early exit/capture/cleanup，next def 同时作为 backedge incoming 与所有正常出口后的实际读取值，且每条抵达后缀的路径完整定义；owner 在 HIR bindings / `carried-locals`，不能由 AST 只改名字。
9. **terminal-arm-vacuous soft phi 尚未恢复为直接分支赋值**：`regress_271` 的四个可继续 arm 都生成 `local tmp = mark(...); result = tmp`，而无值 arm 均先 break/return。Structure 目前没有 value decision plan。关闭此项应让 branch-value planner 只覆盖确实抵达 continuation、且各自产生唯一值的 arm；调用仍在原 arm 执行一次，不跨 cleanup/forward/capture。
