...
8. **Luau generic-for iterator pack 会被纯 state seed 隔开**：`regress_112` 仍生成 `local iter,nil,nil; state=seed; for ... in iter,nil,nil`。只有 gap 不含可观察事件、capture、label、cleanup，且不写 iterator source/alias 时，才能让 `generic-for-iterators` 跨过这段纯状态写并折回 loop header；seed 必须保留原位。
9. **repeat carried 与 live-out identity 尚未合一**：`regress_201` 全 7 方言把源码 `maximum/minimum` 拆成 per-iteration next、loop-carried 与 post-loop result 多套 temp/local。可合并的边界是无 early exit/capture/cleanup，next def 同时作为 backedge incoming 与所有正常出口后的实际读取值，且每条抵达后缀的路径完整定义；owner 在 HIR bindings / `carried-locals`，不能由 AST 只改名字。
10. **terminal-arm-vacuous soft phi 尚未恢复为直接分支赋值**：`regress_271` 的四个可继续 arm 都生成 `local tmp = mark(...); result = tmp`，而无值 arm 均先 break/return。Structure 目前没有 value decision plan。关闭此项应让 branch-value planner 只覆盖确实抵达 continuation、且各自产生唯一值的 arm；调用仍在原 arm 执行一次，不跨 cleanup/forward/capture。
11. **open-tail return 的 fixed-prefix alias pack 未收回**：`common_11_runtime` proto#9 为开放尾调用前的五个稳定 alias 各生成一个 local，再 return。`temp-inline` 可在 alias 未捕获、return 前未写、只被 fixed prefix 各读一次时整包内联，同时保持末尾 open call 的求值顺序与 pack 边界。
12. **effect-only 双空分支仍输出机械控制壳**：`regress_252` 的 `(mark("logic") and false) or 9` 生成 `if mark("logic") then end`。`branch-control` 只能在 predicate 是可合法作为 Lua statement 的 call/method-call、两臂都空且调用必须恰执行一次时改成 Call stmt；不能把任意潜在有副作用表达式泛化成无效 expression statement。
