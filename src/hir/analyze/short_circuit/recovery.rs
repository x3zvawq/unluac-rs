//! 短路 DAG 的表达式恢复与 branch 计划构建。
//!
//! 它依赖 StructureFacts 给好的短路 DAG、value merge 事实和 HIR lowering 上下文，不会
//! 回头重新扫描 CFG 去猜短路形状。
//! 这里负责普通 value-merge 表达式恢复、branch short-circuit plan 和 subject 消费保护；
//! “保留旧值 + 条件写新值”的 conditional reassign 由同级 `conditional_reassign` 负责。
