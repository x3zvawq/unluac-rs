...
fn expr_has_access_base_binding_use(
    ...
    match expr {
        AstExpr::Var(name) if name_matches_binding(name, binding) => access_base,
        AstExpr::FieldAccess(access) => {
            expr_has_access_base_binding_use(&access.base, binding, true)
        }
        AstExpr::IndexAccess(access) => {
        ...
        AstExpr::Unary(unary) => expr_has_access_base_binding_use(&unary.expr, binding, false),
        AstExpr::Binary(binary) => {
            expr_has_access_base_binding_use(&binary.lhs, binding, false)
                || expr_has_access_base_binding_use(&binary.rhs, binding, false)
        }
        AstExpr::LogicalAnd(logical) | AstExpr::LogicalOr(logical) => {
            expr_has_access_base_binding_use(&logical.lhs, binding, false)
                || expr_has_access_base_binding_use(&logical.rhs, binding, false)
        }
        AstExpr::Call(call) => {
        ...
        AstExpr::SingleValue(expr) => expr_has_access_base_binding_use(expr, binding, false),
        AstExpr::TableConstructor(table) => table.fields.iter().any(|field| match field {
            AstTableField::Array(value) => expr_has_access_base_binding_use(value, binding, false),
            AstTableField::Record(record) => {
