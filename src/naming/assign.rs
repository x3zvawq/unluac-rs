...
use crate::hir::{
    ...
    HirProtoRef, HirStmt, HirTableField, HirTableKey, LocalId, ParamId, UpvalueId,
};
...
struct FunctionNamingEvidence {
    ...
    upvalue_capture_sources: Vec<Option<CapturedBinding>>,
...
#[derive(Debug, Clone, PartialEq, Eq)]
struct ClosureCaptureEvidence {
    parent: HirProtoRef,
    captures: Vec<Option<CapturedBinding>>,
}
...
fn assign_names_for_function(
    ...
    for (index, upvalue) in proto.upvalues.iter().enumerate() {
        let candidate = choose_upvalue_candidate(
            proto,
            *upvalue,
            index,
            evidence,
