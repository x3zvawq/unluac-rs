...
// ── 面向外部消费者的指令格式化 ─────────────────────────────
//
// `format_raw_instr` 把任意 dialect 的一条原始指令格式化成
// `"OPCODE A B C"` 风格的紧凑字符串，供 WASM 桥接层序列化给前端。
// 各 dialect 的 debug.rs 有自己更冗长的调试 dump，这里故意只保留
// 操作码名称 + 操作数值，不附加 pc / origin / 额外注解。
...
/// 各 dialect 的 operand 统一用 "field=value" 空格分隔格式输出。
