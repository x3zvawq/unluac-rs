//! 这个模块承载 PUC-Lua 5.x 各版本 parser 共用的协议基础设施。
//!
//! 它不是一个可选 dialect，而是 Lua 5.1 到 5.5 这些真实 dialect 的共享读取骨架。
//! 入口文件只负责导出共享接口；具体实现按 `header / layout / instruction /
//! sections / macros` 分开。`layout` 只提供基于 raw header layout 的读取 helper，
//! 不再维护第二套工作态布局模型。
...
pub(crate) use self::sections::{
    AbsDebugDriver, AbsLineInfoConfig, ClassicDebugDriver, collect_counted, count_u8,
    parse_abs_debug_sections, parse_classic_debug_sections, parse_tagged_literal_pool,
    parse_upvalue_descriptors, parse_upvalues_with_kinds, require_present,
    validate_optional_count_match,
};
pub(crate) use crate::parser::strings::build_raw_string;
